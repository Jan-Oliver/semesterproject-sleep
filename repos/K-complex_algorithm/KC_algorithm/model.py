...
# Fix for the module path change
def fix_sklearn_preprocessing_data():
    import sklearn.preprocessing._data
    import sys
    sys.modules['sklearn.preprocessing.data'] = sklearn.preprocessing._data
    print("Updated sk-learn import path")

fix_sklearn_preprocessing_data()
...
def get_model():
    ...
    # Load and fix the state dictionary keys
    state_dict = torch.load(model_file, map_location=torch.device('cpu'))
    model_state_dict = state_dict['model']
    new_model_state_dict = {}
    for key in model_state_dict.keys():
        new_key = key.replace('gp_layer.variational_strategy.variational_distribution.', 
                              'gp_layer.variational_strategy._variational_distribution.')
...
def score_KCs(C3, Fs, Stages,sleep_stages = [2,3]):
    ...
    # Adapted: The code assumed that all sleep stages are scored with the same length. 
    # This is not true...
    ...
    if len(peaks) == 0 or len(stage_peaks) == 0:
        return peaks, stage_peaks, d, np.array([])  # Handle case where no peaks are found
