...
def findN2peaks(Stages, data, Fs, min, distance):
    """
    ...
    duration = np.round(Stages['dur'].values[2:-2] * Fs).astype('int')

    for j in np.arange(len(onset)):
        low_bound = onset[j]
        upper_bound = onset[j] + duration[j]
    ...
    if p and stages:  # Check if there is anything to stack
